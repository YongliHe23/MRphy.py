...
class SpinArray(object):
    ...
    def applypulse_ss(
        ...
        Optionals:
            ...
            - ``doEmbed``: [t/F], return ``M`` or ``M_``
            - ``doRelax``: [T/f], do relaxation during Bloch simulation.
            - ``betaoff``: [t/F], turn off beta pulse (saturation pulse) if true
            - ``Δf``⊻ ``Δf_``: `(N,*Nd ⊻ nM)`, "Hz", off-resonance.
            - ``b1Map`` ⊻ ``b1Map_``: `(N,*Nd ⊻ nM,xy,(nCoils))`, transmit \
              sensitivity.
        ...
        """
        kwargs={
                'doEmbed':False, 'doRelax':doRelax, 'doUpdate':doUpdate,
                'loc':loc, 'loc_':loc_, 
                'Δf':Δf, 'Δf_':Δf_,
                'b1Map':b1Map, 'b1Map_':b1Map_
                }
        MT_=self.applypulse(pulse, **kwargs)
        MT_=torch.nan_to_num(MT_)#(N,nM,xyz)
        if betaoff:
            Beta_=torch.zeros_like(MT_[...,0])
        else:
            Beta_=torch.zeros_like(MT_[...,0])
            Beta_=torch.arctan(((MT_[...,0]**2+MT_[...,1]**2+1e-8).sqrt())/(MT_[...,2]+1e-8)) #the saturation angle in rad (N,nM)

        T_r=TR-(pulse.dt*pulse.rf.shape[-1]).item() #time from the end of beta to the end of cycle
        E1_=torch.exp(-T_r/self.T1_) #T1 relaxation (N,nM)
    ...
    def applypulse_ss_sms(
        ...
        r"""Calculate the steady state(SS) M for inner-volume saturation SS imaging (SMS EPI)
        The specific sequence for (SS-)SMS EPI is:
        [beta(iv_saturate)--alpha(slice-selective_tip_down)-readout]+[beta--readout]x(vTR/TR-1)
        Inputs:
            - ``pulse``: mrphy.mobjs.Pulse.
        Optionals:
            - ``loc`` ⊻ ``loc_``: `(N,*Nd ⊻ nM,xyz)`, "cm", locations.
            ...
            - ``b1Map`` ⊻ ``b1Map_``: `(N,*Nd ⊻ nM,xy,(nCoils))`, transmit \
              sensitivity.
            - ``TR``:float, Repetition time for beta in the Steady-State sequence.
            - ``vTR``:float, repetition time for alpha, TR for a volume frame
            - ``alpha``: float, Flip angle in degree for the slice-selective tip down pulse.   
            - ``alphaDur``:float, duration of the alpha pulse   
        Outputs:
            - ``M`` ⊻ ``M_``: `(N,*Nd ⊻ nM,xyz)`
        """
        kwargs={
        ...
        MT_=self.applypulse(pulse,b1Map=b1Map, b1Map_=b1Map, **kwargs)
        Tr=TR-(pulse.dt*pulse.rf.shape[-1]).item()-alphaDur #time remaining after beta+alpha to the end of one TR
        Tt=TR-(pulse.dt*pulse.rf.shape[-1]).item()#time remaining after just beta

        MT_=torch.nan_to_num(MT_)#(N,nM,xyz)
        Beta_=torch.zeros_like(MT_[...,0])
        Beta_=torch.arctan(((MT_[...,0]**2+MT_[...,1]**2).sqrt())/MT_[...,2]) #the saturation angle in rad (N,nM)
        E1_=torch.exp(-Tr/self.T1_) #T1 relaxation for Tr (N,nM)
        E1t_=torch.exp(-Tt/self.T1_) #T1 relaxation for Tt (N,nM)
        
        # A_,B_,As_,Bs_ dim=(N,nM)
        A_=torch.cos(Beta_)*E1t_
        B_=self.M_[...,2]*(1-E1t_)
        As_=torch.cos(Beta_)*E1_*math.cos(math.radians(alpha))#A star
        Bs_=self.M_[...,2]*(1-E1_)#B star

        
        Mss_=torch.zeros_like(MT_) #(N,nM,xyz)
        Nrep=vTR/TR
        Nrep_=Nrep-1
        Mss_[...,2]=(A_**Nrep_*Bs_+((1-A_**Nrep_)*B_)/(1-A_))/(1-A_**Nrep_*As_) #steady state M
